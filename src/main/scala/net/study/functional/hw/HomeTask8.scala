...
import net.study.functional.hw.HomeTask8.Product
...
object HomeTask8 extends App {
  ...
  def generatePaymentTokenMixParam(msisdn: String, tempCode: Int, product: Product): PaymentToken = SecurityServer.generatePaymentToken(msisdn, product, tempCode)

  val tokenProviderWithAllParam: String => Int => Product => PaymentToken = (generatePaymentTokenMixParam _).curried

  var processedPayments = new ListBuffer[PaymentResult]()

  serverPool.foreach(server => {
    payments.filter(payment => server.canProcess(payment.productType))
      .map(payment => PaymentResult(payment, server.withdrawPayment(payment, tokenProviderWithAllParam(payment.msisdn)(payment.tempCode))))
      .foreach(result => processedPayments += result)
  })
  ...
  val paymentStatus: PostPaymentDomain = {
  ...
  val result: Seq[PaymentStatus] = processedPayments map {
    paymentStatus
  }
  println(result)
}
